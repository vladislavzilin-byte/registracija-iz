...
export default function FinancePanel({
  ...
  const [rangeStart, rangeEnd, rangeLabel] = useMemo(() => {
    ...
    if (mode === "month") {
      ...
      label = t("finance_month_year", { month: t(`month_${month}`), year });
    } else if (mode === "year") {
      start = new Date(year, 0, 1);
      end = new Date(year + 1, 0, 1);
      label = t("finance_year", { year });
    } else {
      start = new Date(rangeFrom || now);
      end = new Date((rangeTo || now) + "T23:59:59");
  ...
  const combinedItems = useMemo(() => {
    return [...systemItems, ...manualItems]
      .map((item) => ({
        ...item,
        expense: (item.amount || 0) * (percent / 100),
      }))
  ...
  const deleteManual = (id) => setManualEntries((prev) => prev.filter((e) => e.id !== id));
  ...
  const editManual = (item) => {
    ...
    const newDesc = window.prompt(t("finance_edit_desc"), e.description || "");
    if (newDesc === null) return;
    const newAmt = window.prompt(t("finance_edit_amount"), String(e.amount));
    if (newAmt === null) return;
    const amt = Number(newAmt);
    if (!amt || amt <= 0) return;
    const newTime = window.prompt(t("finance_edit_time"), e.time || "");
    if (newTime === null) return;

    setManualEntries((prev) =>
      prev.map((x) =>
        x.id === e.id ? { ...x, description: newDesc, amount: amt, time: newTime } : x
      )
    );
  };
  ...
  const exportPDF = () => {
    ...
    win.document.write(`
      <html><head><title>${t("finance_report_title")}</title><meta charSet="utf-8"/>
      <style>
        body{font-family:system-ui,sans-serif;margin:0;padding:24px;background:#0b1120;color:#f9fafb;}
        .shell{max-width:900px;margin:auto;background:#f9fafb;color:#0f172a;border-radius:18px;overflow:hidden;box-shadow:0 22px 60px rgba(15,23,42,.45);}
        .header{padding:18px 22px;background:linear-gradient(135deg,#020617,#111827);color:#f9fafb;display:flex;justify-content:space-between;align-items:center;}
        .header img{height:68px;}
        .content{padding:16px 22px;font-size:13px;}
        ...
        @media print{body{background:#fff;padding:0;}.shell{box-shadow:none;border:none;}}
      </style></head><body>
      <div class="shell">
        <div class="header">
          <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
              <div style="font-size:18px;font-weight:700;">${settings.masterName || "IZ HAIR TREND"}</div>
              <div style="font-size:11px;opacity:.75;">${t("finance_report_subtitle")}</div>
            </div>
          </div>
        </div>
        <div class="content">
          <div style="margin-bottom:16px;color:#4b5563;font-size:12px;">
            ${t("finance_report_period")}: <b>${rangeLabel}</b>
          </div>
          <div class="summary">
          ...
          <h2 style="font-size:14px;margin:16px 0 8px;">${t("finance_list_title")}</h2>
          <table>
            <thead><tr>
              <th>${t("date")}</th><th>${t("time")}</th><th>${t("description")}</th>
              <th>${t("amount")}</th><th>${t("expenses")}</th><th>ID</th>
            </tr></thead>
            <tbody>
              ${groupedByDate.length === 0
                ? `<tr><td colspan="6" style="text-align:center;color:#9ca3af;">${t("finance_no_records")}</td></tr>`
                : groupedByDate.map(g => g.items.map((item, i) => `
                  <tr>
                    ...
                    <td>${item.type === "system" ? "#" + item.shortId : ""}</td>
                  </tr>`).join("")).join("")}
            </tbody>
          </table>
        </div>
